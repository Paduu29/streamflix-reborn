...
class StreamplayExtractor : Extractor() {

    override val name = "Streamplay"
    override val mainUrl = "https://streamplay.to"
    override val aliasUrls = listOf("https://straemplay.org")
    ...
    override suspend fun extract(link: String): Video {
        ...
        val result = resolver.getResult(
            ...
            completion = { currentUrl, pageHtml, _ ->
                currentUrl.contains("/embed-", ignoreCase = true) &&
                    pageHtml.contains("jwplayer", ignoreCase = true)
            },
            shouldAllowNavigation = { url, isMainFrame ->
                if (!isMainFrame) true else isAllowedStreamplayNavigation(url)
            },
    ...
    private fun extractVideo(html: String, requestedUrl: String, playerState: String?): Video {
        ...
        val source = extractSource(candidates)
            ?: throw IllegalStateException("No Streamplay media source found")
    ...
    private fun isAllowedStreamplayNavigation(url: String): Boolean {
        val host = Uri.parse(url).host?.lowercase().orEmpty()
        if (host.isBlank()) return false
        if (host == "streamplay.to" || host == "www.streamplay.to") return true
        if (host == "straemplay.org" || host == "www.straemplay.org") return true
